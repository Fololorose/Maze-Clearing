...
def print_info(current_step, current_node, current_bin_size, current_bin_weight, remaining_rubbish_coordinates, explored, frontier, children, solution):
    output = f"""
...
{'Bin moved to the disposal room.' if current_node.coordinate in DISPOSAL_ROOMS else 'Bin collected rubbish in the room.' if current_node.rubbish_size or current_node.rubbish_weight else 'Bin moved to an empty room.'}
...
def greedy(state_space, ROOMS_WITH_RUBBISH, DISPOSAL_ROOMS, initial_node):
  ...
  while not goal_achieved:
    ...
    print_info(current_step, current_node, current_bin_size, current_bin_weight, [room[0] for room in ROOMS_WITH_RUBBISH], explored, frontier, children, solution)
